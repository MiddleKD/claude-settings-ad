 ...
 * Stop hook — 세션 상태 저장
 *
 * 매 응답 종료 시 실행 (async). transcript에서 최근 3턴(user+assistant)과
...
function extractFromTranscript(transcriptPath) {
  ...
  try {
    ...
    process.stderr.write(`[session-end] transcript 읽기 실패: ${err.message}\n`);
  }

  // 전체 턴을 entries로 펼친 뒤 마지막 MAX_ENTRIES개만 유지
...
let transcriptPath = null;
  ...
  try {
    ...
    process.stderr.write(`[session-end] 세션 상태 저장: ${sessionFile}\n`);
  } catch (err) {
    process.stderr.write(`[session-end] 저장 실패: ${err.message}\n`);
  }
